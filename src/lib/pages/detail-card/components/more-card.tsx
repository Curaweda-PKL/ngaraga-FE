...
// Memoized CardItem to prevent unnecessary re-renders
const CardItem: React.FC<{ card: Card; onClick: () => void }> = memo(({ card, onClick }) => (
  <div
    key={card.id}
    onClick={onClick}
...
export const MoreCardSection: React.FC = memo(() => {
  ...
  if (error || cards.length === 0) {
    return (
      <div className="w-full mb-10 lg:ml-8 flex flex-col items-center justify-center w-full h-[400px]">
        <svg
          xmlns="http://www.w3.org/2000/svg"
  ...
  return (
    <div className="w-full mb-10 lg:ml-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center sm:items-start">
        {/* Cards Grid */}
        <div className="grid gap-6 px-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 flex-grow ml-2 lg:mr-6">
          {cards.map((card) => (
            <CardItem
              key={card.id}
              card={card}
